...
Sub Activity_Create(FirstTime As Boolean)
	...
	logoutbtn.Initialize("logoutbtn")
	logoutbtn.Background=mycode.btnbg(False)
	logoutbtn.Text=mycode.buttoncsb(False,Chr(0xF08B),True)
	appbar.AddView(logoutbtn,appbar.Width-mycode.appbarheight,0,mycode.appbarheight,mycode.appbarheight)
...
Sub profile As Panel
	...
	pwall.SetBackgroundImage(LoadBitmap(File.DirAssets,"profile_bg.webp"))
...
Sub blockitem(name as String,profile_pic As String,i As Int) As Panel
	Dim p As Panel
	p.Initialize("")
	Dim namelb As Label
	...
	namelb.TextSize = mycode.textsize(8)
	...
	unblockbtn.Background = mycode.btnbgdynamic(mycode.bgColor,Colors.DarkGray,10dip)
	unblockbtn.TextColor=Colors.White
	...
	p.AddView(unblockbtn,namelb.Left+namelb.Width+10dip,0,100dip,40dip)
	p.Height = 40dip
	Return p 
End Sub
...
Sub logoutbtn_click
	...
	If File.Exists(File.DirInternal,"user") Then
		...
		If File.Exists(File.DirInternal,"block") Then
			File.Delete(File.DirInternal,"block")
		End If
