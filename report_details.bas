...
Sub OtherMessage(profile As String,sms As String,name As String,id As String,top As Int) As Panel
	Dim p As Panel
	p.Initialize("p")
...
Sub reportItem As Panel
	...
	Dim cv As Panel
	cv.Initialize("")
	cv.Color=mycode.bgColor
	cv.Elevation= 5dip
	Dim cd As ColorDrawable
	cd.Initialize(0xFF492D6F,10dip)
	cv.Background=cd
	Dim pbase As Panel
	pbase.Initialize("")
	...
	btnreport.Background = mycode.btnbgdynamic(mycode.naviColor,mycode.bgColor,10dip)
