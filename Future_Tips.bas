...
Sub Globals
	...
	Dim offstring As String = Chr(0xF204)
	Dim onstring As String = Chr(0xF205)
	Dim hqb As Boolean
End Sub
...
Sub Activity_Create(FirstTime As Boolean)
	...
	swithbtn.Initialize("switchbtn")
	appbar.AddView(swithbtn,appbar.Width-appbar.Height,0,appbar.Height,appbar.Height)
	swithbtn.Background =mycode.btnbg(False)
	
	swithbtn.Typeface = Typeface.FONTAWESOME
	If hqb = True Then
		swithbtn.Text = onstring
		papergetter
		
		Else
		swithbtn.Text = offstring
		papergetterlow
	End If
	swithbtn.TextSize = mycode.textsize (8)
	
	
	swithbtn.TextColor=Colors.White
...
Sub quality As Boolean
	Dim r As Boolean
	If File.Exists(File.DirInternal,"qcheck") Then
		Dim a As String =  File.ReadString(File.DirInternal,"qcheck")
		If a = "true" Then
		r = True
			Else
				r = False
		End If
	Else
		File.WriteString(File.DirInternal,"qcheck","true")
		r = True
	End If
	Return r
End Sub
...
Sub changequality(b As Boolean)
	Select b
		Case True
			swithbtn.Text = offstring
			File.WriteString(File.DirInternal,"qcheck","false")
			Activity.Finish
			StartActivity(Me)
			Case False
				swithbtn.Text = onstring
...
Sub papergetter
	...
	j.Download(Main.newsite&"/futurepaper/getallpaper/high")
End Sub

Sub papergetterlow
	Dim j As HttpJob
	j.Initialize(papergetters,Starter)
	j.Download(Main.newsite&"/futurepaper/getallpaper/low")
End Sub
...
'	menubtn.Tag=img 'ls.Get(ASViewPager1.CurrentIndex)
...
Sub menu_click
'	Dim b As Button = Sender
'	Dim img As ImageView  = b.Tag
'	Dim bmp  As Bitmap  = img.Bitmap
'	If bmp.IsInitialized Then
'		Dim bx As B4XView = img
'		Dim bb As B4XBitmap = bx.Snapshot
'		mycode.AddBitmapToGallery1(bb,DateTime.Now&".png","image/png")
'		
'		Else
'			ToastMessageShow("not success",False)
'	End If
	
	saveimage(setList.Get(ASViewPager1.CurrentIndex))
'	Dim b As Button = Sender
'	
'	Savemsg(b.Tag)
End Sub
